...
\par 
Because labels are provided at the set/video level (not per repetition), we aggregate multiple repetitions from the same volunteer into a single subject-level sample. Each repetition window is first encoded into an embedding $e_r \in \mathbb{R}^d$.\wg{how do u generate this embedding?} We then perform masked attention pooling to handle variable $N_{\text{reps}}$:
\begin{equation}
...
\par 
Two coaches providd scores for a given set, we use a reliability-weighted fusion:
\begin{equation}
y = 0.25\,y_{C1} + 0.75\,y_{C2}.
\end{equation}

\par 
We train and evaluate the assessment model separately per exercise and per view (front/side) under subject-disjoint splits, and report MAE on the 0--10 scale.\wg{above u mentioned MSE,
is this the difference between training and testing?? or just a typo?}
...
\par
To demonstrate the system's ability to synthesize recognition confidence and aspect-level quality scores into actionable natural language advice, we present a qualitative case study. The following output was generated for a 12-rep \textit{Dumbbell Shoulder Press} set where the assessment module detected progressive form degradation due to fatigue.

\par
\textbf{Input Context:}
\begin{itemize}
    \item \textbf{Exercise:} Dumbbell Shoulder Press (Confidence: 92\%)
    \item \textbf{Aspect Scores:} Elbow Path dropping from 8.5 (initial) to 6.6 (final); Top Position stability decreasing in reps 10--12.
    \item \textbf{Detected State:} Fatigue Onset.
\end{itemize}
