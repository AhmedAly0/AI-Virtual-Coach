"""Exercise Recognition: Pose-based MLP for exercise recognition.
...
Training Functions:
    - train_exercise_recognition: Single run training
    - train_exercise_recognition_multi_run: Multiple runs with different seeds for robust evaluation
...
Config files:
    - config/exer_recog_baseline_front.yaml (front view - baseline 19 features)
    - config/exer_recog_baseline_side.yaml (side view - baseline 19 features)
    - config/exer_recog_specialized_front.yaml (front view - 37 features)
    - config/exer_recog_specialized_side.yaml (side view - 37 features)
"""
...
def _save_multi_run_summary(
    ...
    with open(summary_path, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("EXERCISE RECOGNITION MULTI-RUN SUMMARY\n")
        f.write("=" * 80 + "\n\n")
...
def train_exercise_recognition(
    npz_path: str,
    config_path: str = 'config/exer_recog_baseline_front.yaml',
    *,
    ...
    if results_folder is None:
        ...
        logger.info("Exercise recognition (pose MLP) results directory: %s", results_folder)
...
def train_exercise_recognition_multi_run(
    npz_path: str,
    config_path: str = 'config/exer_recog_baseline_front.yaml'
    ...
    if not multi_run_cfg.get('enabled', False):
        raise ValueError(
            ...
            "or use train_exercise_recognition() for single runs."
        )
    ...
    for run_number in range(1, num_runs + 1):
        ...
        run_results = train_exercise_recognition(
            npz_path=npz_path,
    ...
    logger.info("\n" + "=" * 80)
    logger.info("MULTI-RUN EXERCISE RECOGNITION COMPLETE")
    logger.info("=" * 80)
...
# ============================================================================
train_experiment_1 = train_exercise_recognition
train_experiment_1_multi_run = train_exercise_recognition_multi_run
